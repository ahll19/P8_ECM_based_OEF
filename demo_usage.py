...
if __name__ == '__main__':
    ...
    model, modeling_time = ies.optimize_lazy_explicit_uec_model(maxiter=args.maxiter, lp_torlence=args.lp_tolerance)
    ...
    description_str = f"Instance: {args.instance_file}\n" \
                      ...
                      f"modeling time: {modeling_time:.2f} seconds\n" \
                      f"Epsilon: {args.epsilon}\n" \
                      f"optimal operation cost: {ies.get_optimal_operation_cost():.2f}"
    save_name = f"{args.instance_file}/{args.cut_off}_{args.maxiter}_{args.lp_tolerance}_{args.epsilon}"
